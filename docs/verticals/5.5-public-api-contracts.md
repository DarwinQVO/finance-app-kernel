...
**Domain 4: Research (Academic Paper Repository)**

**Use Case:** University repository uses API to extract citations from papers

**API Flow:**
```bash
# 1. Upload research paper PDF
curl -X POST https://api.truthconstruction.com/v1/uploads \
  -H "Authorization: Bearer tc_live_university_abc" \
  -F "file=@paper.pdf" \
  -F "parser_type=academic_paper"

# 2. Retrieve citations as entities
curl https://api.truthconstruction.com/v1/entities?upload_id=upl_abc123&type=citation \
  -H "Authorization: Bearer tc_live_university_abc"
# Response: [{"entity_id": "ent_1", "type": "citation", "title": "...", "authors": [...], ...}]
```
